...
void FACE::extractFeature(const cv::Mat& cropFace, int id, std::vector<float>& feature)
{
   Preprocessing(cropFace);
   cv::Mat image=img_;

   if (id == 4)
   	image.convertTo(image, CV_32FC3, 0.0078125,-127.5*0.04);
   else
   	image.convertTo(image, CV_32FC3, 0.0078125,-127.5*0.0078125);

   boost::shared_ptr<Net<float> > net = nets_[id];
   Blob<float>* input_layer = net->input_blobs()[0];
...
void FACE::detect_net(int i)
{
    ...
    // avoid 0 0 0 0 bbox
    std::vector<cv::Rect> temp_bboxs;
    for (int i = 0; i < bounding_box_.size(); i++) {
        if (bounding_box_[i].width != 0 && bounding_box_[i].height != 0)
            temp_bboxs.push_back(bounding_box_[i]);
    }
    if (temp_bboxs.size() != bounding_box_.size())
        bounding_box_ = temp_bboxs;
...
void FACE::detectFromOriImg(const cv::Mat& img, const cv::Mat& imgOri, std::vector<cv::Mat>& cropFace)
{
    // clear network output
    regression_box_temp_.clear();
    bounding_box_.clear();
	...
	int imOriWidth = imgOri.cols;
	int imOriHeight = imgOri.rows;
	float ratio = min(imOriWidth/float(resizeX_), imOriHeight/float(resizeY_));
	//cout << "FACE.cpp Ratio = " << ratio << endl;
   ...
   for(int j = 0; j < alignment_.size(); j++)
   {
        ...
        srctemp[0] = src5Points[j][0]*ratio;
        srctemp[1] = src5Points[j][1]*ratio;
        srctemp[2] = src5Points[j][2]*ratio;
        srctemp[3] = src5Points[j][3]*ratio;
        srctemp[4] = src5Points[j][4]*ratio;
...
void FACE::calculateDistance(std::vector<float> output1, std::vector<float> output2, int id, float& l2)//modify
{
 	if (id == 3)  {
		    ...
		    for(int i = 0; i < 128; i++)//for normalization
		    {
		        output1[i] /= sqrtsum1;
		        output2[i] /= sqrtsum2;
		    }
		    ...
		    for(int i = 0; i < 512; i++)//for normalization
		    {
		        output1[i] /= sqrtsum1;
		        output2[i] /= sqrtsum2;
		    }
