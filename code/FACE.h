...
public:
    ...
    void detectFromOriImg(const cv::Mat& img, const cv::Mat& imgOri, std::vector<cv::Mat>& cropFace);
    ...
    void extractFeature(const cv::Mat& cropFace, int id, std::vector<float>& feature);
    void calculateDistance(std::vector<float> output1, std::vector<float> output2,int id,float& l2);
