...
int main(int argc, char **argv)
{
 ...
 //       FACE face_net(model_file);
  //  }
	   ...
	   if (argc<8){
			 std::cout<<"usage "<<argv[0]<< " image_list_file  output_result_file  threshold_value param3_result_file  success_face_dir  id_path  photo_path failed_face_dir"<<std::endl;
			 return 0;
		 }
        ...
        finish_a=clock();
           ...
           img_src2=cv::imread(img_b_paths[j]);
				...
				if( extruct_fg_a && extruct_fg_b){
					 Euclidean_distance1 = RemoveDuplicate::GetInstance()->CheckDuplicate2(crop_a_faces[0],crop_b_faces[0]);
					 output_file<<id_name <<"\t"<<name_list[j]<<"\t"<<Euclidean_distance1<<endl;
					 //cout<<id_name <<"\t"<<name_list[j]<<"\t"<<img_b_paths[j]<<endl;
					 per_name=name_list[j]+"-0";
					  ...
					  if(Euclidean_distance1 <= THRESHOLD){
						     success_match+=1;
								 ...
								 if(RS_32 && SV_IMG){
									 ...
									 SaveImage(img_dir_save,per_name,crop_b_faces[0]);
								 }
					  }
						else{
							//per_name=name_list[j]+"-"+std::to_string(k);
							if(RS_32 && SV_IMG){
								resize(crop_b_faces[0],temp_crop,cv::Size(32,32));
								SaveImage(img_dir_save,per_name,temp_crop);
							}
							else if(SV_IMG){
								SaveImage(img_dir_save,per_name,crop_b_faces[0]);
